...
RainyDay.prototype.rain = function(presets) {
	this.initialForce = function(fps) { //传入0表示重置，否则递增
		if (fps) {
			if (fps == -1) {
				this.options.fps = this.defaults.fps;
			} else {
	...
	this.addDropCallback = function(dt, ctx) {
		...
		ctx.drawImage(this.clearbackground, 0, 0, this.width, this.height);
		ctx.globalAlpha = this.options.blurOpacity;
